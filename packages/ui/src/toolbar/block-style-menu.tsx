...
export function BlockStyleMenu({ editor, t }: { editor: Editor; t?: Translate }) {
  const ctxT = useT();
  const tr = t ?? ctxT;
  ...
  return (
    <DropdownMenu>
      <Tooltip>
        ...
        <TooltipContent side="bottom">{tr("toolbar.blockStyle")}</TooltipContent>
      </Tooltip>
      <DropdownMenuContent align="start" className="tk-w-36">
        {BLOCK_STYLES.map((item) => {
          const Icon = item.icon;
          return (
            <DropdownMenuItem key={item.labelKey} onClick={() => item.run(editor)}>
