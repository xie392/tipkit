...
| **依赖注入** | 图片 / 附件上传、KaTeX 渲染、AI provider、i18n 由消费方通过 `EditorDeps` 注入，内核零外部服务依赖 |
| **Markdown 即时转换** | 粘贴 Markdown、输入 `#` `- ` `` ` `` 即时转节点；IME 组合输入兜底转换（链接、行内代码） |
| **斜杠菜单** | `/` 唤起可搜索分组面板，22 种内容节点开箱即用（含 AI、脚注、视频、画板等） |
| **高级节点** | 表格（内联热区 + 只读列宽 + 全选）、KaTeX、分栏、折叠块、目录、附件、视频、图片块、代码块高亮（含 Mermaid 渲染） |
| **AI 生成** | 内置 AI 流式生成扩展与浮层，`EditorDeps.ai` 注入 provider，headless 命令层可独立调用 |
| **协作增强** | 脚注、划词评论、UniqueID（块级节点自动 id）、状态标签、Emoji 完整库（`:xxx` 短代码） |
| **文档工具** | 查找替换（装饰高亮）、LanguageTool 语法检查（检查函数可注入）、Mermaid 图表 |
